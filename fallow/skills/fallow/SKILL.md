...
Runtime source-map confidence for cloud-backed runtime tools:
...
| `null` + `null` | The row predates FunctionIdentity v2 or no source-map step ran. | Treat the row as missing confidence metadata. Do not downgrade it to `low` without other evidence. |
