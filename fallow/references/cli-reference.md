...
| `--ownership` | bool | `false` | Attach ownership signals to hotspot entries: bus factor (Avelino truck factor), contributor count, top contributor with stale-days, recent contributors (top-3), declared CODEOWNERS owner, ownership drift, unowned-hotspot detection. JSON adds `low-bus-factor`, `unowned-hotspot`, `ownership-drift` action types. Implies `--hotspots`. Requires git. |
...
# Hotspots with ownership signals (bus factor, declared owner, drift)
fallow health --format json --quiet --hotspots --ownership

# Ownership with hashed emails (regulated environment)
fallow health --format json --quiet --hotspots --ownership --ownership-emails hash
